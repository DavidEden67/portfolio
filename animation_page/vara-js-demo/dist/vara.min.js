Vara = function(t, e, s, i) {
    ...
    for (var s in t = document.createElementNS("http://www.w3.org/2000/svg", t), e) t.setAttributeNS(null, s.replace(/[A-Z]/g, function(t, e, s, i) {
    ...
    for (var t = this.analyseWidth(), e = 0; e < this.texts.length; e++) {
        for (var s = [], i = ("string" == typeof this.texts[e] ? {
            ...
            if (t.breakPoints[e][r].length > 0)
                for (var a, o = t.breakPoints[e][r].length, h = 0; h <= o; h++) {
                    var l = t.breakPoints[e][r][h];
                    t.breakPoints[e][r][o - 1] != n.length && t.breakPoints[e][r].push(n.length);
                    var c = null == t.breakPoints[e][r][h - 1] ? 0 : t.breakPoints[e][r][h - 1];
    ...
    for (var a = 0, o = !0, h = 0; h < this.texts.length; h++) {
        var l = [],
            c = 0;
        t = this.texts[h].fontSize;
        var p = this.texts[h].strokeWidth,
            u = null == this.texts[h].color ? null == this.properties.color ? "black" : this.properties.color : this.texts[h].color,
            d = null == this.texts[h].duration ? null == this.properties.duration ? 2e3 : this.properties.duration : this.texts[h].duration;
        this.texts[h].duration = d;
        var x = null == this.texts[h].id ? h : this.texts[h].id;
        this.texts[h].delay = null == this.texts[h].delay ? 0 : this.texts[h].delay, this.prevDuration += this.texts[h].delay, null == this.texts[h].fromCurrentPosition && (this.texts[h].fromCurrentPosition = {
            x: !0,
            y: !0
        }), o = null == this.texts[h].y || !(null != this.texts[h].fromCurrentPosition.y && !this.texts[h].fromCurrentPosition.y);
        var f = u,
            g = null == this.texts[h].textAlign ? this.properties.textAlign : this.texts[h].textAlign;
        n = null == this.texts[h].lineHeight ? n : this.texts[h].lineHeight / t;
        var y = this.createNode("g", {
            class: "outer",
            transform: "translate(0,0)",
            "data-text": this.texts[h].text
        });
        s.appendChild(y), r = a;
        for (var m = 0, v = 0; v < this.texts[h].text.length; v++) {
            var w = 0,
                C = this.createNode("g");
            y.appendChild(C);
            for (var S = 0, b = 0, k = 0; k < this.texts[h].text[v].length; k++) {
                var B = this.texts[h].text[v][k],
                    A = this.createNode("g");
                C.appendChild(A);
                i = 0;
                ...
                u = " " == B ? "transparent" : f, P.forEach(function(t, s) {
                    ...
                    A.appendChild(i), i.setAttribute("transform", "translate(" + t.mx + "," + -t.my + ")"), S = S > t.my - i.getBBox().y ? S : t.my - i.getBBox().y, 0 == s && (b = S), i.style.opacity = 0, i.style.strokeDasharray = i.getTotalLength() + " " + (i.getTotalLength() + 2), i.style.strokeDashoffset = i.getTotalLength() + 1
                }), l.push(A);
                i = A.getBBox().x * t;
                var V = this.texts[h].letterSpacing;
                "object" == typeof V && (V = void 0 === V[B] ? void 0 === V.global ? 0 : V.global : V[B]), A.getBBox().width < this.texts[h].minWidth && (i -= (this.texts[h].minWidth - A.getBBox().width) / 2), A.setAttribute("transform", "translate(" + (w - i + V) + ",0)  scale(" + t + ")"), w += A.getBBox().width * t + V, A.getBBox().width < this.texts[h].minWidth && (w += this.texts[h].minWidth - A.getBBox().width)
            }
            var W = C.getBBox();
            C.setAttribute("transform", "translate(" + p * t + "," + (-W.y + p * t) + ")");
            var D = 0;
            "center" == g ? D = (this.svg.getBoundingClientRect().width - W.width) / 2 : "right" == g && (D = this.svg.getBoundingClientRect().width - W.width), this.setPosition(C, {
                x: D,
                y: c + this.contents.p.tf * t - S
            }), c += n * t, m += this.contents.p.tf * t + (this.contents.p.tf * t - b)
        }
        null != this.texts[h].y && 1 != this.texts[h].fromCurrentPosition.y || this.setPosition(y, {
            y: r
        }), null != this.texts[h].fromCurrentPosition && this.texts[h].fromCurrentPosition.y && (a += null == this.texts[h].y ? 0 : this.texts[h].y), this.setPosition(y, {
            x: this.texts[h].x,
            y: this.texts[h].y
        }, this.texts[h].fromCurrentPosition), o && (a += m), null != this.drawnCharacters[x] && (x = h), this.drawnCharacters[x] = {
            characters: l,
            queued: this.texts[h].queued,
            container: y,
            index: h
        }, (null == this.texts[h].autoAnimation || this.texts[h].autoAnimation) && this.properties.autoAnimation && (e.draw(x, d), (null == this.texts[h].queued || this.texts[h].queued) && (e.prevDuration += d))
    }
    ...
    for (var a = 0; a < this.texts.length; a++) {
        var o, h = this.texts[a];
        o = "string" == typeof h.text ? [h.text] : h.text, this.texts[a].text = o, this.texts[a].letterSpacing = null == this.texts[a].letterSpacing ? null == this.properties.letterSpacing ? 0 : this.properties.letterSpacing : this.texts[a].letterSpacing, this.texts[a].strokeWidth = null == this.texts[a].strokeWidth ? null == this.properties.strokeWidth ? this.contents.p.bsw : this.properties.strokeWidth : this.texts[a].strokeWidth;
        var l = null == this.texts[a].breakWord ? null != this.properties.breakWord && this.properties.breakWord : this.texts[a].breakWord,
            c = null == h.fontSize ? null == this.properties.fontSize ? this.fontSize : this.properties.fontSize : h.fontSize;
        ...
        var u, d = null == this.texts[a].width ? e : this.texts[a].width,
            x = [],
            f = null == this.texts[a].x ? 0 : this.texts[a].x;
        this.trueFontSize = c, this.texts[a].fontSize = p;
        for (var g = this.texts[a].letterSpacing, y = 0; y < o.length; y++) {
            for (var m = f, v = [], w = o[y], C = 0, S = 0; S < w.length; S++)
                if ("object" == typeof g && "object" == typeof g && (g = void 0 === g[w] ? void 0 === g.global ? 0 : g.global : g[w]), null != this.characters[w[S].charCodeAt(0)] ? ((u = this.characters[w[S].charCodeAt(0)].w * p) < this.texts[a].minWidth && (u += p * (this.texts[a].minWidth - this.characters[w[S].charCodeAt(0)].w) / 2), u += g) : " " == w[S] ? (u = this.space.w * p, C = m) : u = this.questionMark.w * p + g, m + (u += this.texts[a].strokeWidth * p) >= d) {
                    0 == C && (l = !0);
                    var b = S;
                    " " == w[S] || l || (b = w.slice(0, b + 1).search(/\S+$/)), v.push(b), m = f + m - C
                } else t += u, m += u;
            x.push(v)
        }
        s.push(x)
...
}, "undefined" != typeof module ? module.exports = Vara : window.Vara = Vara;